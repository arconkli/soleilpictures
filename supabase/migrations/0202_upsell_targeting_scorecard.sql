-- 0202_upsell_targeting_scorecard.sql
--
-- Extends the upsell scorecard so the pitch-TARGETING change is measurable.
--
-- Until now every exposure counted the same, so "nobody converts" and "nobody
-- was ever asked" were indistinguishable: the denominator was dominated by
-- users who had just signed up and had no reason to want the product yet. The
-- client now suppresses those exposures (lib/upsellEligibility.js) and records
-- the suppression as `up_suppressed`, plus elig/elig_reason/pressure on the
-- exposure envelope.
--
-- Two new top-level keys:
--   targeting — how many pitches were withheld, why, and the CTA rate among
--               exposures that were actually aimed at someone
--   cap       — the card wall: blocks, server-side rejections (the regression
--               detector for the previously-swallowed 42501), toast view→click
--
-- Both signatures are unchanged, so boardsApi.js and RevenueView.jsx need no
-- wiring change. Body is 0198's with additive edits only.
--
-- Counters are FIXED `filter` expressions, never jsonb_object_agg over a
-- caller-supplied prop: analytics_events INSERT is anon-open, so props is
-- untrusted input. That contract is why 0198 exists; it is preserved here.
-- Rates are deliberately NOT computed server-side — raw numerator/denominator
-- only, so the client's RateCell owns small-N gating.
  ...
  return (
    with ev as (
      ...
      where e.occurred_at >= v_since
        and e.event in ('up_exposure_summary', 'up_feature_hover',
                        ...
                        'up_suppressed', 'up_cap_toast_cta', 'card_create_blocked')
    ...
    groups as (
      select surface, header,
             ...
             count(*) filter (where props->>'elig' = 'true')          as elig_n,
             count(*) filter (where props->>'elig' = 'true'
                                and props->>'outcome' = 'cta')        as elig_cta
    ...
    feat as (
      -- Which Creator pitch lines get read, per group. `row` is the index into
      -- CREATOR_FEATURES; `key` its stable slug (CREATOR_FEATURE_KEYS). The
      -- row filter still accepts 0-4 even though the list is now 3 long: rows
      -- 3/4 exist in history, and dropping them would rewrite the past.
    ...
    select json_build_object(
      ...
      'targeting', (
        select json_build_object(
          'suppressed',         count(*) filter (where event = 'up_suppressed'),
          'suppressed_users',   count(distinct user_id) filter (where event = 'up_suppressed'),
          'by_reason', json_build_object(
            'no_cards',      count(*) filter (where event = 'up_suppressed' and props->>'reason' = 'no_cards'),
            'same_day',      count(*) filter (where event = 'up_suppressed' and props->>'reason' = 'same_day'),
            'below_floor',   count(*) filter (where event = 'up_suppressed' and props->>'reason' = 'below_floor'),
            'low_intensity', count(*) filter (where event = 'up_suppressed' and props->>'reason' = 'low_intensity'),
            'cap_unknown',   count(*) filter (where event = 'up_suppressed' and props->>'reason' = 'cap_unknown')),
          'by_surface', json_build_object(
            'chip',         count(*) filter (where event = 'up_suppressed' and props->>'surface' = 'chip'),
            'first_value',  count(*) filter (where event = 'up_suppressed' and props->>'surface' = 'first_value'),
            'list_toolbar', count(*) filter (where event = 'up_suppressed' and props->>'surface' = 'list_toolbar')),
          'eligible_exposures', count(*) filter (where event = 'up_exposure_summary' and props->>'elig' = 'true'),
          'eligible_users',     count(distinct user_id) filter (where event = 'up_exposure_summary' and props->>'elig' = 'true'),
          'eligible_cta',       count(*) filter (where event = 'up_exposure_summary'
                                                   and props->>'elig' = 'true'
                                                   and props->>'outcome' = 'cta'))
        from ev
      ),
      'cap', (
        select json_build_object(
          'blocked',          count(*) filter (where event = 'card_create_blocked'
                                                 and props->>'reason' in ('demo_cap', 'demo_cap_cell', 'server_cap')),
          'blocked_users',    count(distinct user_id) filter (where event = 'card_create_blocked'
                                                 and props->>'reason' in ('demo_cap', 'demo_cap_cell', 'server_cap')),
          -- Non-zero means the client gate and the server trigger disagree.
          -- These were silently swallowed before; any sustained count is a bug.
          'server_rejects',   count(*) filter (where event = 'card_create_blocked'
                                                 and props->>'reason' = 'server_cap'),
          'near_toast_views', count(*) filter (where event = 'up_cap_toast_view'),
          'near_toast_ctas',  count(*) filter (where event = 'up_cap_toast_cta'),
          'cap_hit_exposures', count(*) filter (where event = 'up_exposure_summary' and header = 'cap-hit'),
          'cap_hit_users',     count(distinct user_id) filter (where event = 'up_exposure_summary' and header = 'cap-hit'))
        from ev
      ),
    ...
    from (
      select e.occurred_at,
             json_build_object(
               ...
               -- Targeting state at exposure time. `elig` is a tri-state: true,
               -- false, or null for rows written before the envelope carried it
               -- (comparing to the literal 'true' would flatten null to false
               -- and silently backdate the change).
               'elig',        case when e.props ? 'elig' and e.props->>'elig' is not null
                                   then (e.props->>'elig' = 'true') end,
               'elig_reason', left(coalesce(e.props->>'elig_reason', ''), 24),
               'pressure',    left(coalesce(e.props->>'pressure', ''), 12),
