-- 0197: Upsell behavior scorecard + exposure feed over the up_* family.
--
-- The client now instruments every Creator upsell surface (PricingModal, the
-- signed-in and public /pricing pages, the chip / Settings / cap-toast entry
-- points — see boards/src/lib/upsellMetrics.js) with the up_* event family.
-- The analyzable unit is up_exposure_summary: ONE dense row per exposure with
-- outcome, dismiss method, dwell, time-to-first-interaction, feature-row
-- reads, toggle sequence, rage/dead clicks, and the exposure envelope
-- {surface, header, via, copy_rev, exposure_n, tier, cap_pct, acct_days}.
--
-- Two read layers:
--   • admin_upsell_scorecard — aggregates. `groups` rows come EXCLUSIVELY from
--     up_exposure_summary (+ up_feature_hover), keyed surface × header — the
--     summary is the true exposures denominator (pricing_view is
--     once-per-pageload and under-counts SPA re-opens). `funnel` keeps the
--     classic per-surface counters (pricing_view / intents / checkout_open /
--     abandons) as continuity columns — checkout_open carries no header, so
--     these are keyed by surface only rather than force-joined onto groups.
--     `by_copy` nests summary outcomes per copy_rev inside each group so a
--     copy revision is before/after-comparable without an A/B test.
--   • admin_upsell_exposures — the qualitative feed: recent summaries, newest
--     first. At the current volume an admin can read every single exposure.
--
-- Honesty contract with the UI: raw numerators/denominators only — the client
-- RateCell/small-N layer computes and gates every rate. up_trace is
-- deliberately absent from every event filter so trace batches are never
-- scanned here. props are anon-writable: every numeric goes through a regex
-- cast-guard, enum-shaped strings are counted with fixed FILTER lists (never
-- jsonb_object_agg on raw values), and free strings are length-clamped.
-- subscription_started (server-fired) may attribute one user to several
-- surfaces if they were exposed on more than one — irrelevant at current
-- volume, noted here for honesty.
  ...
  return (
    with ev as (
      select e.session_id, e.user_id, e.event, e.props, e.occurred_at,
             coalesce(nullif(e.props->>'surface', ''), 'unknown')   as surface,
             coalesce(nullif(e.props->>'header', ''), 'generic')    as header,
             coalesce(nullif(e.props->>'copy_rev', ''), 'legacy')   as copy_rev
    ...
    groups as (
      select surface, header,
             ...
             percentile_cont(0.5) within group (order by
               case when (props->>'dwell_ms') ~ '^[0-9]+(\.[0-9]+)?$'
                    then (props->>'dwell_ms')::numeric end)     as med_dwell_ms,
             percentile_cont(0.5) within group (order by
               case when (props->>'ttfi_ms') ~ '^[0-9]+(\.[0-9]+)?$'
                    then (props->>'ttfi_ms')::numeric end)      as med_ttfi_ms,
             percentile_cont(0.5) within group (order by
               case when (props->>'exposure_n') ~ '^[0-9]+$'
                    then (props->>'exposure_n')::numeric end)   as med_exposure_n,
             count(*) filter (where (props->>'toggles_n') ~ '^[0-9]+$'
                                and (props->>'toggles_n')::int > 0)  as toggled_n,
             count(*) filter (where (props->>'feat_ms') ~ '^[0-9]+$'
                                and (props->>'feat_ms')::int > 0)    as feat_read_n,
             sum(case when (props->>'rage_n') ~ '^[0-9]+$' then (props->>'rage_n')::int else 0 end) as rage_n,
             sum(case when (props->>'dead_n') ~ '^[0-9]+$' then (props->>'dead_n')::int else 0 end) as dead_n,
...
-- ── The qualitative feed: recent exposures, one line each ─────────────────────
    ...
    from (
      select e.occurred_at,
             json_build_object(
               ...
               'toggles_n',   case when (e.props->>'toggles_n')   ~ '^[0-9]+$' then (e.props->>'toggles_n')::int end,
               'dwell_ms',    case when (e.props->>'dwell_ms')    ~ '^[0-9]+$' then (e.props->>'dwell_ms')::int end,
               'ttfi_ms',     case when (e.props->>'ttfi_ms')     ~ '^[0-9]+$' then (e.props->>'ttfi_ms')::int end,
               'feat_ms',     case when (e.props->>'feat_ms')     ~ '^[0-9]+$' then (e.props->>'feat_ms')::int end,
               'cta_hes_ms',  case when (e.props->>'cta_hes_ms')  ~ '^[0-9]+$' then (e.props->>'cta_hes_ms')::int end,
               'price_hes_ms',case when (e.props->>'price_hes_ms')~ '^[0-9]+$' then (e.props->>'price_hes_ms')::int end,
               'rage_n',      case when (e.props->>'rage_n')      ~ '^[0-9]+$' then (e.props->>'rage_n')::int end,
               'dead_n',      case when (e.props->>'dead_n')      ~ '^[0-9]+$' then (e.props->>'dead_n')::int end,
               'exposure_n',  case when (e.props->>'exposure_n')  ~ '^[0-9]+$' then (e.props->>'exposure_n')::int end,
               'cap_pct',     case when (e.props->>'cap_pct')     ~ '^[0-9]+$' then (e.props->>'cap_pct')::int end,
               'acct_days',   case when (e.props->>'acct_days')   ~ '^[0-9]+$' then (e.props->>'acct_days')::int end,
