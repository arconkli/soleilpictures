...
import { Spark as VizSpark } from './viz/TrendLine.jsx';
...
// Tiny sparkline with the MIN_POINTS honesty floor. Kept as a thin adapter so
// existing callers can keep passing [{v}]; the drawing (and the floor) now live
// in viz/TrendLine, which is plain SVG and gap-aware.
export function Spark({ data, color }) {
  return (
    <div className="admin-stat-spark">
      <VizSpark values={(data || []).map((d) => d?.v)} color={color} />
    </div>
  );
}
