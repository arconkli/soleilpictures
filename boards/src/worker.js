...
export default {
  async fetch(request, env) {
    ...
    // Proxy eligible ring users to the staging Worker (latest build) — but ONLY
    // the static SPA shell + hashed /assets/* (isStaticSpaPath). Everything the
    // prod Worker handles specially (/api/*, SEO/sitemap, public /c + /share +
    // /explore) stays on prod, so the staging Worker only ever serves static
    // files and therefore needs ZERO secrets (no service-role / OpenAI key).
    // Pure crypto check, no Supabase round-trip. The staging Worker itself
...
// True only for the static SPA shell + hashed assets — the sole things we proxy
// to the (secret-free) staging Worker. Every route the prod Worker handles
// specially (/api/*, sitemaps, public /c + /share + /explore, IndexNow key)
// returns false so it stays on prod, where the service-role / OpenAI secrets
// live. This is what lets the staging Worker run with no secrets at all.
function isStaticSpaPath(url) {
  const p = url.pathname;
  if (p.startsWith('/api/')) return false;
  if (p.startsWith('/c/')) return false;
  if (p.startsWith('/share/')) return false;
  if (p === '/explore' || p === '/explore/') return false;
  if (p === '/sitemap.xml' || p === '/sitemap-images.xml') return false;
  if (/^\/[a-z0-9]+\.txt$/i.test(p)) return false; // IndexNow ownership key file
  return true; // '/', /board/*, /pricing, /legal/*, /assets/*, all app routes
}
