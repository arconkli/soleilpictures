...
// Standalone version: floating gear button + ⌘. / Ctrl-. shortcut, no host
...
const __TWEAKS_STYLE = `
  .twk-gear{position:fixed;right:14px;bottom:14px;z-index:2147483645;
    width:32px;height:32px;border-radius:50%;
    border:1px solid var(--line-2,#2a2a2e);background:var(--bg-1,#131315);
    color:var(--ink-2,#8a8b91);cursor:pointer;
    display:grid;place-items:center;
    box-shadow:0 4px 14px rgba(0,0,0,.28);
    transition:color .15s, transform .15s, background .15s;
    padding:0}
  .twk-gear:hover{color:var(--ink-0,#f5f5f6);background:var(--bg-act,#1f1f23);transform:rotate(20deg)}
  .twk-gear svg{display:block}
...
export function useTweaks(defaults) {
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    ...
    setValues((prev) => {
      ...
      try { localStorage.setItem(__TWEAKS_STORAGE_KEY, JSON.stringify(next)); } catch (_) {}
      return next;
    });
  }, []);
