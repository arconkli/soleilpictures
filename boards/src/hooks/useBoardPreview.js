  ...
  try {
    ...
    if (workerPromise) {
      try {
        ...
        data = workerData;
        perf.bump('preview.decode.workerUsed');
        perf.mark('preview.decode.worker.ms', workerMs);
      } catch (e) {
        if (perf.isEnabled()) console.warn('[perf] preview decode worker failed; inline fallback', e?.message || e);
        const inline = _decodeInline(b64);
        data = inline.data;
        perf.bump('preview.decode.inlineFallback');
        perf.mark('preview.decode.inline.ms', inline.ms);
      }
    } else {
      const inline = _decodeInline(b64);
      data = inline.data;
    ...
    const _decodeMs = performance.now() - _tDec0;
    perf.mark('preview.decode.ms', _decodeMs);
    if (perf.isEnabled()) console.log('[perf] preview decoded', boardId, _decodeMs.toFixed(1) + 'ms', data.cards.length + ' cards');
