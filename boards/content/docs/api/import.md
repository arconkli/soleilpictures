...
```sh
curl -X POST {{fact:siteOrigin}}/api/v1/boards/$BOARD/import \
...
stamped with an identifier in the `{{fact:importSourceScope}}` scope holding the
...
```sh
curl "{{fact:siteOrigin}}/api/v1/resolve?scope={{fact:importSourceScope}}&value=https://cdn.example.com/ref/diner-01.jpg" \
  -H "Authorization: Bearer {{fact:tokenPrefix}}…"
```
...
|---|---|
| An image format the API stores ({{fact:apiCardKinds}} covers the card kinds; see [Images](/docs/api/images)) | An **image card** — the bytes are copied into your workspace |
...
- At most **{{fact:maxImportItems}} items** per call.
