...
The board header's **Share** button does not open it — that button copies a
view-only link in one click, which is what most sharing is. You reach the panel
from **Manage access** on the toast that confirms the copy, from `⌘K` → "Share
this cluster", or by right-clicking any cluster in the sidebar and choosing
...
**Share** in the board header is one button and it copies. One click puts a
view-only link on your clipboard, refreshes the preview image the link will
unfurl with, and offers **Manage access** if you wanted the panel after all.

Boards you are viewing without edit rights show a clear VIEW ONLY badge, so
there is never ambiguity about whether your changes will stick — and since you
cannot mint a link there, **Share** opens the panel instead.
