...
At most 10 redirect URIs. They must be `https`,
...
The code is good for 120 seconds and is **single-use**. It is bound to
...
| | |
|---|---|
| **Access token** | 60 minutes, sent as `Authorization: Bearer …` |
| **Refresh token** | Rotates on every use; valid 90 days from last use |
| **Scopes** | `delete` · `read` · `write` — the same three [scopes](/docs/api/authentication) a token has |
